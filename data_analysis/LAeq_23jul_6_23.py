...
print("Sound file not on github b/c of its size. Link to file on request.")

chunk_duration = 10 # secs

f = wavfile.read("/tmp/MONO-23jul_6_23_16kHz.wav")
fs = f[0]
y = f[1] / 32768.0
...
customdate = datetime.datetime(2022, 7, 23, 6, 23)
...
for i in range(len(y)//chunk - 1):
    db = LAeq_math.Leq(y[i*chunk:(i+1)*chunk],fs)
